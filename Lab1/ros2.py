...
node.move_to_pose({'joint_wrist_yaw':np.radians(20)},blocking = True)

node.move_to_pose({'joint_wrist_pitch':np.radians(20)},blocking = True)

node.move_to_pose({'joint_wrist_roll':np.radians(20)},blocking = True)

node.move_to_pose({'joint_gripper_finger_left': 0.5}, blocking=True)
...
node.move_to_pose({'joint_head_pan': current_pan + np.radians(35)}, blocking=True)

current_tilt = node.joint_state.position[node.joint_state.name.index('joint_head_tilt')]
node.move_to_pose({'joint_head_tilt': current_tilt + np.radians(-35)}, blocking=True)
